...
bool PhysBody::Contains(int x, int y) const
{
	...
	for (b2Fixture* p = body->GetFixtureList(); p!= nullptr; p=p->GetNext())
	{
		if (p->GetShape()->TestPoint(body->GetTransform(), point))
			return true;
	}
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	b2Vec2 point(PIXEL_TO_METERS(x1), PIXEL_TO_METERS(y1));
